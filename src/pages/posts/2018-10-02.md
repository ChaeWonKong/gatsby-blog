...
굉장히 긴 내용이 프린트 되었다. 스크롤을 위로 올려보면 html 코드를 가져온 것을 알 수 있다.
...
['First Log  ', ' 10 August, 2017']
```
XX.split("-") 는 문자열을 하이픈("-")을 기준으로 쪼개고 그 쪼갠 결과물을 리스트에 저장하게 하는 메소드(Method)이다.

    프론트엔드와 백엔드에 대하여1  - 30 September, 2018
보다시피 제목과 날짜는 하이픈으로 나뉘어 있다.
우리는 이 하이픈을 기중으로 한 줄이었던 문자열을 두 줄로 나누어 리스트에 넣은 것이다.
